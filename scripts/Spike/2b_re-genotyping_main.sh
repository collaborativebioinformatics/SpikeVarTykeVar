...
# Input vairables are: VARIANT, VAF, VCF_1, VCF_2, MOD_BAM, REF_FASTA, OUTPUT_DIR, READ_LENGTH
# VARIANT can be either SNV or SV
# READ_LENGTH can be either SHORT or LONG for SVs

# Example command:
# For SNV:
# ./2b_re-genotyping_main.sh SNV 0.1 /path/to/vcf1.vcf /path/to/vcf2.vcf /path/to/mod.bam /path/to/ref.fasta /path/to/output_dir
# or 
# For SV:
...
if [ "$VARIANT" = "SNV" ]; then
    echo "Input is 'SNV'"
    ./2b_SNV.sh $READ_LENGTH $VAF $MERGED_VCF $MOD_BAM $REF_FASTA $OUTPUT_VCF
